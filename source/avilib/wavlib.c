...
*/
size_t plat_read(int fd, char *buf, size_t count)
{
	size_t n = 0, r = 0;
...
* automatically restart after a recoverable interruption
*/
size_t plat_write(int fd, const char *buf, size_t count)
{
	size_t n = 0, r = 0;
...
#define ftruncate plat_ftruncate
...
#define plat_malloc(x)	_plat_malloc(__FILE__, __LINE__, x)
#define plat_zalloc(x)	_plat_zalloc(__FILE__, __LINE__, x)
#define plat_realloc(x,y)	_plat_realloc(__FILE__, __LINE__, x, y)
...
static size_t wav_bswap_fdwrite(int fd, const uint8_t *buf, size_t len)
{
...
size_t wav_read_data(WAV handle, uint8_t *buffer, size_t bufsize)
{
    size_t r = 0;
...
size_t wav_write_data(WAV handle, const uint8_t *buffer, size_t bufsize)
{
    size_t w = 0;
